...
pv = solph.components.Source(
    ...
    outputs={bus_electricity: solph.Flow(
        ...
        nominal_value=solph.Investment(
            ep_costs=(
                economics.annuity(capex=par.PV_CAPEX_PANELS,   n=par.LIFETIME, wacc=par.WACC, u=par.PV_LIFETIME_SYSTEM)
                + economics.annuity(capex=par.PV_CAPEX_INVERTER, n=par.LIFETIME, wacc=par.WACC, u=par.PV_LIFETIME_INVERTER)
                + par.PV_OPEX
            ),
...
wind = solph.components.Source(
    ...
    outputs={bus_electricity: solph.Flow(
        fix=timeseries["Wind"],
        nominal_value=solph.Investment(
            ep_costs=economics.annuity(capex=par.WIND_CAPEX, n=par.LIFETIME, wacc=par.WACC, u=par.WIND_LIFETIME_SYSTEM) + par.WIND_OPEX,
            existing=0,
        ),
        variable_costs=0,
    )},
...
electrolyser = solph.components.Converter(
    ...
    inputs={bus_electricity: solph.Flow(
        nominal_value=solph.Investment(
            ep_costs=(
                economics.annuity(capex=par.ELECTROLYSER_CAPEX_SYSTEM, n=par.LIFETIME, wacc=par.WACC, u=par.ELECTROLYSER_LIFETIME_SYSTEM)
                + economics.annuity(capex=par.ELECTROLYSER_CAPEX_STACK,  n=par.LIFETIME, wacc=par.WACC, u=par.ELECTROLYSER_LIFETIME_STACK)
...
heatpump = solph.components.Converter(
    ...
    inputs={bus_electricity: solph.Flow(
        nominal_value=solph.Investment(
            ep_costs=(
                economics.annuity(capex=par.HEATPUMP_CAPEX, n=par.LIFETIME, wacc=par.WACC, u=par.HEATPUMP_LIFETIME)
...
fuelcell = solph.components.Converter(
    ...
    inputs={bus_H2: solph.Flow(
        nominal_value=solph.Investment(
            ep_costs=(
                economics.annuity(capex=par.FUELCELL_CAPEX_SYSTEM, n=par.LIFETIME, wacc=par.WACC, u=par.FUELCELL_LIFETIME_SYSTEM)
                + economics.annuity(capex=par.FUELCELL_CAPEX_STACK,  n=par.LIFETIME, wacc=par.WACC, u=par.FUELCELL_LIFETIME_STACK)
...
thermalstorage = solph.components.GenericStorage(
    ...
    nominal_storage_capacity=solph.Investment(
        ep_costs=(
            economics.annuity(capex=par.THERMALSTORAGE_CAPEX, n=par.LIFETIME, wacc=par.WACC, u=par.THERMALSTORAGE_LIFETIME)
...
print("Heat pump input (MWh/h):", heatpump_input.sum())
...
print("Battery charge (MWh/h):", bat_charge.sum())
bat_discharge = results[(battery, bus_electricity)]['sequences']['flow']
print("Battery discharge (MWh/h):", bat_discharge.sum())
...
print("Salt cavern charge (MWh/h):", saltcavern_charge.sum())
...
unit_map = {
    ...
    "heatpump":              ("Heat Pump",                  "MW"),
    "fuelcell":              ("Fuel Cell",                  "MW"),
    "battery_energy":        ("Battery (energy capacity)",  "MWh"),
    "battery_power":         ("Battery (power capacity)",   "MW"),
    "saltcavern_energy":     ("Salt Cavern (energy)",       "MWh"),
    "saltcavern_power":      ("Salt Cavern (power)",        "MW"),
    "thermalstorage_energy": ("Thermal Storage (energy)",   "MWh"),
    "thermalstorage_power":  ("Thermal Storage (power)",    "MW"),
}

print("=" * 50)
print("  OPTIMAL INVESTMENT SIZES")
print("=" * 50)
for key, (label, unit) in unit_map.items():
    print(f"  {label:<35} {invest_sizes[key]:>8.1f}  {unit}")
print("=" * 50)
...
flow_specs = [
    ...
    (0,  2,  pv_flow.sum(),              "PV → El bus"),
    (1,  2,  wind_flow.sum(),            "Wind → El bus"),

    (2,  3,  el_input.sum(),             "El bus → Electrolyser"),
    (2,  7,  heatpump_input.sum(),       "El bus → Heat Pump"),
    (2,  8,  bat_charge.sum(),           "El bus → Battery (charge)"),
    (2,  11, el_demand_ts.sum(),         "El bus → El Demand"),
    (2,  14, curtailment_el.sum(),       "El bus → El Curtailment"),

    (5,  2,  fuelcell_output_el.sum(),   "fuelcell → El bus"),
    (8,  2,  bat_discharge.sum(),        "Battery (discharge) → El bus"),

    (3,  4,  el_output.sum(),            "Electrolyser → H₂ bus"),
    (3,  6,  el_output_heat.sum(),       "Electrolyser → Heat bus (waste)"),

    (4,  5,  fuelcell_input.sum(),       "H₂ bus → fuelcell"),
    (4,  9,  saltcavern_charge.sum(),    "H₂ bus → Salt Cavern (charge)"),
    (4,  12, h2_demand_ts.sum(),         "H₂ bus → H₂ Demand"),
    (4,  15, curtailment_h2.sum(),       "H₂ bus → H₂ Curtailment"),

    (9,  4,  saltcavern_discharge.sum(), "Salt Cavern (discharge) → H₂ bus"),

    (7,  6,  heatpump_output.sum(),      "Heat Pump → Heat bus"),
    (5,  6,  fuelcell_output_heat.sum(),      "fuelcell → Heat bus"),
    #(6,  10, thermalstorage_charge,    "Heat bus → Thermal Storage (charge)"),
    (6,  13, heat_demand_ts.sum(),       "Heat bus → Heat Demand"),
    (6,  16, curtailment_heat.sum(),     "Heat bus → Heat Curtailment"),
...
monthly_flex_h2 = pd.DataFrame({
    "Electrolyser":     el_output.values,  # TODO: have it as electrolyser H2 output minus fuelcell H2 consumption
...
with mpl.rc_context({
    ...
    for col, (charge, discharge, p_cap, label, color) in enumerate(active):
        ...
        ax.text(-0.07, 0.98, f"({chr(97 + col)})",
...
ep_costs_map = {
    ...
    "wind": (
        economics.annuity(capex=par.WIND_CAPEX, n=par.LIFETIME, wacc=par.WACC, u=par.WIND_LIFETIME_SYSTEM)
        + par.WIND_OPEX
    ),
    "electrolyser": (
        economics.annuity(capex=par.ELECTROLYSER_CAPEX_SYSTEM, n=par.LIFETIME, wacc=par.WACC, u=par.ELECTROLYSER_LIFETIME_SYSTEM)
    ...
    "saltcavern_energy": (
        economics.annuity(capex=par.SALTCAVERN_CAPEX,             n=par.LIFETIME, wacc=par.WACC, u=par.SALTCAVERN_LIFETIME)
        + economics.annuity(capex=par.SALTCAVERN_CAPEX_CUSHION_GAS, n=par.LIFETIME, wacc=par.WACC, u=par.SALTCAVERN_LIFETIME)
        + par.SALTCAVERN_OPEX
    ),
    "saltcavern_power": (
        economics.annuity(capex=par.SALTCAVERN_CAPEX_COMPRESSOR, n=par.LIFETIME, wacc=par.WACC, u=par.SALTCAVERN_LIFETIME_COMPRESSOR)
    ),
...
print(f"Electrolyser output split: {electro_frac_h2:.2%} H2, {electro_frac_heat:.2%} heat")
...
print(f"Fuel Cell output split: {fuelcell_frac_el:.2%} electricity, {fuelcell_frac_heat:.2%} heat")
...
with mpl.rc_context({
    ...
    # ── Doughnut chart (same style, 1.5× radius) ──────────────────────────────
    fig_d, ax_d = plt.subplots(figsize=(12, 8))  # 1.5× the pie figsize

    wedges_d, texts_d, autotexts_d = ax_d.pie(
        ordered_values,
        labels=None,
        colors=ordered_colors,
        autopct=lambda pct: f"{pct:.1f}%" if pct >= threshold else "",
        startangle=90,
        pctdistance=0.82,
        wedgeprops={"linewidth": 0.8, "edgecolor": "white", "width": 0.5},  # width<1 makes it a doughnut
        radius=1.5,
    )

    for autotext in autotexts_d:
        autotext.set_fontsize(20)
        autotext.set_fontfamily("serif")

    small_wedges_d = sorted(
        [(wedge, value) for wedge, value in zip(wedges_d, ordered_values)
         if value / total * 100 < threshold],
        key=lambda x: x[1], reverse=True
    )

    for k, (wedge, value) in enumerate(small_wedges_d):
        pct = value / total * 100
        mid = np.deg2rad((wedge.theta1 + wedge.theta2) / 2)

        r_elbow = 1.5 * 1.15 + k * 0.12   # scale elbow by radius
        r_text  = 1.5 * 1.25 + k * 0.12   # scale text  by radius

        xA, yA = 1.5 * 0.95 * np.cos(mid), 1.5 * 0.95 * np.sin(mid)
        xB, yB = r_elbow * np.cos(mid),     r_elbow * np.sin(mid)
        xT, yT = r_text  * np.cos(mid),     r_text  * np.sin(mid)
    ...
    ax_d.axis('equal')
    plt.tight_layout()
    func.savefigure(fig_d, "MODEL_cost_breakdown_donut")
    plt.show()
    ...
    inner_wedges, _, inner_autotexts = ax_d.pie(
        ordered_values,
        labels=None,
        colors=ordered_colors,
        autopct=lambda pct: f"{pct:.1f}%" if pct >= threshold else "",
        startangle=90,
        pctdistance=0.75,
        wedgeprops={"linewidth": 0.8, "edgecolor": "white", "width": 0.50},
        radius=1.0,
    )
    ...
    # ── Leader lines for small component slices (pointing inward) ─────────────
    small_component_wedges = sorted(
        [(wedge, value) for wedge, value in zip(inner_wedges, ordered_values)
         if value / total * 100 < threshold],
        key=lambda x: x[1], reverse=True,
    )

    n_small      = len(small_component_wedges)
    angle_spread = 70   # degrees between adjacent small-slice labels

    # Diverging offsets: 0, +22, -22, +44, -44, ...
    # so labels always fan away from each other
    offsets = [+22, -22]
    for i in range(n_small):
        step      = (i + 1) // 2
        direction = 1 if i % 2 == 0 else -1
        offsets.append(direction * step * angle_spread)

    for k, ((wedge, value), angle_offset) in enumerate(
        zip(small_component_wedges, offsets)
    ):
        pct     = value / total * 100
        mid_deg = (wedge.theta1 + wedge.theta2) / 2
        lbl_deg = mid_deg + angle_offset
        mid_rad = np.deg2rad(mid_deg)
        lbl_rad = np.deg2rad(lbl_deg)

        r_start = 0.50   # where the line begins (inner edge of ring)
        r_line  = 0.24   # where the line ends
        r_text  = 0.16   # where the number is drawn — further inward than line tip

        xS, yS = r_start * np.cos(mid_rad), r_start * np.sin(mid_rad)
        xE, yE = r_line  * np.cos(lbl_rad), r_line  * np.sin(lbl_rad)  # line endpoint
        xT, yT = r_text  * np.cos(lbl_rad), r_text  * np.sin(lbl_rad)  # text position

        ax_d.annotate(
            "",
            xy=(xS, yS), xycoords="data",
            xytext=(xE, yE),             # ← line ends at xE, yE, not xT, yT
            textcoords="data",
            arrowprops=dict(arrowstyle="-", color="grey", lw=0.8),
        )
        ax_d.text(
            xT, yT, f"{pct:.1f}%",      # ← text drawn at xT, yT, further inward
            ha="center", va="center",
            fontsize=15, fontfamily="serif", color="black",
        )

    # ── Outer ring: group breakdown ───────────────────────────────────────────
    group_definitions = {
        "Supply\n(PV + Wind)":      ["PV", "Wind"],
        "PtX\n(El. + HP + FC)":     ["Electrolyser", "Fuel Cell", "Heat Pump"],
        "Storage\n(Bat. + Cavern)": ["Battery", "Salt Cavern"],
    }
    group_colors_inner = {
        "Supply\n(PV + Wind)":      "#55A868",
        "PtX\n(El. + HP + FC)":     "#4C72B0",
    ...
    label_to_value    = dict(zip(ordered_labels, ordered_values))
    group_values      = []
    group_labels_list = []
    group_colors_list = []
    for grp_label, members in group_definitions.items():
        grp_val = sum(label_to_value.get(m, 0) for m in members)
        group_values.append(grp_val)
        group_labels_list.append(grp_label)
        group_colors_list.append(group_colors_inner[grp_label])

    outer_wedges, *_ = ax_d.pie(
        group_values,
        labels=None,
        colors=group_colors_list,
        autopct=None,
        startangle=90,
        wedgeprops={"linewidth": 0.8, "edgecolor": "white", "width": 0.45},
        radius=1.55,
    )

    # ── Labels centred inside each outer ring slice ────────────────────────────
    for wedge, grp_label, grp_val in zip(
        outer_wedges, group_labels_list, group_values
    ):
        pct     = grp_val / total * 100
        mid     = np.deg2rad((wedge.theta1 + wedge.theta2) / 2)
        r_label = 1.10 + (1.55 - 1.10) / 2   # radial midpoint of outer band

        xT = r_label * np.cos(mid)
        yT = r_label * np.sin(mid)

        short = group_short_names[grp_label]
        ax_d.text(
            xT, yT,
            f"{short}\n{pct:.1f}%",
            ha="center", va="center",
    ...
    ax_d.axis("equal")
    plt.tight_layout()
    func.savefigure(fig_d, "MODEL_cost_breakdown_donut_total")
    plt.show()
...
with mpl.rc_context({
    ...
    ax.bar(x, cb, width, label="Initial Cost", color="steelblue")
    ax.bar(x, ct, width, label="Replacement Cost",          color="firebrick", bottom=cb)
    ax.set_xticks(x)
...
roi       = (npv_final) / capex_total if capex_total > 0 else float('inf')
...
print(f"  ROI                : {roi*100 if not np.isnan(roi) else float('nan'):>10.2f} %")
print("=" * 52)
...
with mpl.rc_context({
    ...
    ax.bar(0, disc_capex / 1e6,
           color="firebrick", alpha=0.8, label="CAPEX (discounted)", zorder=2)
    ax.bar(years[1:], disc_annual / 1e6,
           color="steelblue", alpha=0.8, label="Net cashflow (discounted)", zorder=2)
    ax.axhline(0, color="black", lw=0.8, ls="--")

    # ── NPV line ──────────────────────────────────────────────────────────────
    ax.plot(years, npv_series / 1e6,
            color="black", lw=1.5, ls="--", marker="o", markersize=3,
            zorder=5, label="Cumulative NPV")
    ax.axhline(0, color="black", lw=0.5, ls=":")

    if payback_discounted is not None:
        ax.axvline(payback_discounted, color="black", lw=0.8, ls=":",
                   label=f"Payback Time= {payback_discounted:.1f} years")

    # ── Formatting ────────────────────────────────────────────────────────────
    npv_vals = npv_series / 1e6
    cf_vals  = np.concatenate([[disc_capex], disc_annual]) / 1e6
    all_vals = np.concatenate([npv_vals, cf_vals])
    pad  = 0.05
    span = all_vals.max() - all_vals.min()
    ax.set_ylim(all_vals.min() - pad * span, all_vals.max() + pad * span)

    ax.set_ylabel("Cumulative NPV and \nDiscounted Cash Flow  / [M€]")
    ax.set_xlabel("Year")
    ax.xaxis.set_major_locator(mticker.MaxNLocator(integer=True))
...
flex_curtailment = {
    ...
    **func.curtailment_metrics(curtailment_h2,   h2_demand_ts.sum(),  "h2"),
    **func.curtailment_metrics(curtailment_heat, heat_demand_ts.sum(),"heat"), 
}
...
flex_system = {
    ...
    "sc_ratio_h2_to_el_and_heat":    func.sector_coupling_ratio(fuelcell_input.sum(), h2_demand_ts.sum()), #FIXME:simply wrong
...
'''ptx_radar = {
    ...
    "fuelcell": {
        "Capacity\nfactor":          cf_fuelcell,
        #"Avg Δ Load":                fuelcell_ramp_util,  
        "Output\npotential":          fuelcell_ramp_util,
