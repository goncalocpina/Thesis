...
# SCENARIO DEFINITION
#=============================================================================
SCENARIO_NAME = "H2_ONLY"   # ← change per run
...
'''# =============================================================================
# USE ONLY IF YOU WANT TO ENFORCE A SPECIFIC WIND CAPACITY (e.g., for sensitivity analysis)
# CONSTRAINT — Wind capacity must be a multiple of WIND_TURBINE_RATED_POWER
# =============================================================================

WIND_TURBINE_RATED_POWER = 200  # MW per turbine

# Add integer variable directly to the Pyomo model
model.wind_turbines = po.Var(domain=po.NonNegativeIntegers)

# Find the correct investment key for the wind flow
# oemof InvestmentFlowBlock.invest is indexed by (node_in, node_out, 0)
wind_invest_key = None
for key in model.InvestmentFlowBlock.invest:
    node_in, node_out, _ = key   # keys are 3-tuples: (from, to, period)
    if hasattr(node_in, 'label') and node_in.label == 'wind':
        wind_invest_key = key
        break

if wind_invest_key is None:
    raise ValueError("Wind investment variable not found in InvestmentFlowBlock.")

print(f"Wind invest key found: {wind_invest_key}")

# Add constraint: wind_invest == n_turbines * rated_power
model.wind_capacity_constraint = po.Constraint(
    expr=model.InvestmentFlowBlock.invest[wind_invest_key]
         == model.wind_turbines * WIND_TURBINE_RATED_POWER
)
'''
...
axes[1].set_ylabel(r"H$_2$ Demand / [MWh$_{H_2}$ / h]")
